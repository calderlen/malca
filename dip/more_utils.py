...
def LocateLC(folder):
    ''' 
    scan dir tree and extract folders that look like lc*_cal
    '''
...
def plot_multiband(dfv, dfg, ra, dec, peak_option=False):
    ...
    peak, meanmag, length = peak_search_residual_baseline(dfg)
...
def plot_light_curve(df, ra, dec, peak_option=False):
    ...
    """
    cust_id = custom_id(ra,dec)
    peak, meanmag, length = peak_search_residual_baseline(df)
